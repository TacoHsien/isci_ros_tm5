...
void KinectClass::KinectInitial()
{
	if (KinectStatus == false){
		...
		if (eResult == XN_STATUS_OK){
			std::cout << "���T�GKinectSensor Intial Correct!" << std::endl;
			KinectStatus = true;
		}
	}
	//else{
		//std::cout << "ĵ�i�G�Фŭ��ƶ}��Kinect!" << std::endl;
	//}
}
...
void KinectClass::CloseKinect()
{
	if (KinectStatus == true){
		...
		if ( eResult == XN_STATUS_OK ){
			mContext.Shutdown();
			Sleep(10);
			std::cout << "���T�GKinectSensor Shutdown!" << std::endl;
			KinectStatus = false;
		}
		else{
		...
		std::cout << "ĵ�i�G�|���}��KinectSensor!" << std::endl;
	}
}
...
void KinectClass::CaptureImage()
{
	...
	{
		std::cout << "���~ : �|���}��KinectSensor!!" << std::endl;
	}
}
